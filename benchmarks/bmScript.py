...
for measure in measures.items():
    ...
    for size in bm_size:
        ...
        l.append(x.get_benchmark(measure[1](),short_setA,short_setB)[0])

        # Benchmark for short-medium
        l.append(x.get_benchmark(measure[1](),short_setA, medium_setA)[0])

        # Benchmark for short-long
        l.append(x.get_benchmark(measure[1](),short_setA, long_setA)[0])

        # Benchmark for medium-medium
        l.append(x.get_benchmark(measure[1](),medium_setA, medium_setB)[0])

        # Benchmark for medium-long
        l.append(x.get_benchmark(measure[1](),medium_setA, long_setA)[0])

        # Benchmark for long-long
        l.append(x.get_benchmark(measure[1](),long_setA, long_setB)[0])

        temp_df = pd.DataFrame({str(size):l},index=new_index)
        # print(temp_df)
